...
class BaseDatabaseFinding(WriteHatBaseModel):
    ...
    @property
    def categoryLeaf(self):

        try:
            return self.category.name
        except Exception:
            return self.categoryFull
...
class CVSSFinding(BaseDatabaseFinding):
    ...
    def cvss_base_vector(self):
        ...
        cvss_data = self.cvss
        return (
            f'AV:{cvss_data._AV}/AC:{cvss_data._AC}/PR:{cvss_data._PR}/'
            f'UI:{cvss_data._UI}/S:{cvss_data._S}/C:{cvss_data._C}/I:{cvss_data._I}/A:{cvss_data._A}'
        )


    @property
    def cvss_base_metrics(self):
        cvss_data = self.cvss

        return {
            'attack_vector': cvss_data._AV,
            'attack_complexity': cvss_data._AC,
            'privileges_required': cvss_data._PR,
            'user_interaction': cvss_data._UI,
            'scope': cvss_data._S,
            'confidentiality': cvss_data._C,
            'integrity': cvss_data._I,
            'availability': cvss_data._A,
        }
