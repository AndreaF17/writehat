...
from writehat.lib.findingForm import CVSSEngagementFindingForm, CVSS4EngagementFindingForm, DREADEngagementFindingForm, ProactiveEngagementFindingForm
...
class BaseFindingGroup(WriteHatBaseModel):
    ...
    prefix = models.CharField(verbose_name="Finding Prefix", max_length=50, default='F', null=True, blank=True)
        ...
        if self._finding_objects is None:
            ...
            for i, finding in enumerate(findings):
                if (self.report is None) or (not self.report.finding_uuids) or (UUID(str(finding.id)) in self.report.finding_uuids):
                    ...
                    finding.prefix = prefix
                    finding.number = f'{prefix}{i+1:03d}'
...
class CVSSFindingGroup(BaseFindingGroup):
    ...
    prefix = models.CharField(verbose_name="Finding Prefix", max_length=50, default='T', null=True, blank=True)


class CVSS4FindingGroup(BaseFindingGroup):

    findingClass = CVSS4EngagementFinding
    scoringType = models.CharField(default='CVSS4', editable=False, max_length=50)
    prefix = models.CharField(verbose_name="Finding Prefix", max_length=50, default='T4', null=True, blank=True)
...
class DREADFindingGroup(BaseFindingGroup):
    ...
    prefix = models.CharField(verbose_name="Finding Prefix", max_length=50, default='NT', null=True, blank=True)
...
class ProactiveFindingGroup(BaseFindingGroup):
    ...
    prefix = models.CharField(verbose_name="Finding Prefix", max_length=50, default='P', null=True, blank=True)
