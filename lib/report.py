...
class BaseReport(WriteHatBaseModel):
    ...
    tlpDisplay = models.BooleanField(default=True)
    tlpLevel = models.CharField(max_length=20, choices=tlpChoices, default='CLEAR')
    defaultLanguage = models.CharField(max_length=8, choices=defaultLanguageChoices, default='en')

    @classmethod
    def new(cls, name, components=None, engagementParent=None, status='active', tlpDisplay=True, tlpLevel='CLEAR', defaultLanguage='en'):
        ...
        reportModel.tlpDisplay = bool(tlpDisplay)
        reportModel.tlpLevel = cls.normalizeTlpLevel(tlpLevel)
        reportModel.defaultLanguage = cls.normalizeDefaultLanguage(defaultLanguage)
        #reportModel.save()
    ...
    @classmethod
    def normalizeTlpLevel(cls, level, fallback='CLEAR'):

        valid_levels = [c[0] for c in cls.tlpChoices]
        if level in valid_levels:
            return level

        if fallback in valid_levels:
            return fallback
    ...
    @classmethod
    def normalizeDefaultLanguage(cls, language, fallback='en'):

        valid_languages = [c[0] for c in cls.defaultLanguageChoices]

        if language in valid_languages:
            return language

        if fallback in valid_languages:
            return fallback
    ...
    def tlpMarking(self):
        ...
        return f"TLP:{self.normalizeTlpLevel(self.tlpLevel)}"


    @property
    def tlpCssClass(self):
        ...
        return css_classes[self.normalizeTlpLevel(self.tlpLevel)]
    ...
    def update(self, componentJSON=None, name=None, pageTemplate=None, status=None, tlpDisplay=None, tlpLevel=None, defaultLanguage=None):
        ...
        if tlpDisplay is not None:
            log.debug(f"Setting report.tlpDisplay to {tlpDisplay}")
            self.tlpDisplay = bool(tlpDisplay)

        if tlpLevel is not None:
            normalized_level = self.normalizeTlpLevel(tlpLevel, fallback=self.tlpLevel)
            log.debug(f"Setting report.tlpLevel to {normalized_level}")
            self.tlpLevel = normalized_level

        if defaultLanguage is not None:
            normalized_language = self.normalizeDefaultLanguage(defaultLanguage, fallback=self.defaultLanguage)
            log.debug(f"Setting report.defaultLanguage to {normalized_language}")
            self.defaultLanguage = normalized_language
...
#           log.setLevel(logLevel)
    ...
    def render(self):
        ...
        rendered = master_template.render({ 
            ...
            'severity_colors': getattr(settings, 'VULNERABILITY_SEVERITY_COLORS', {}),
        })
...
class Report(BaseReport):
    ...
    def update(self, componentJSON=None, reportName=None, pageTemplate=None, findings=None, status=None, tlpDisplay=None, tlpLevel=None, defaultLanguage=None):
        '''
        Do everything that the parent function does, and also update findings
        '''

        super().update(componentJSON, reportName, pageTemplate, status, tlpDisplay=tlpDisplay, tlpLevel=tlpLevel, defaultLanguage=defaultLanguage)
...
class SavedReportImportForm(forms.Form):
    ...
    tlpOverride = forms.BooleanField(
        label='Override TLP from Template',
        required=False,
        initial=False
    )
    tlpDisplay = forms.TypedChoiceField(
        label='TLP Marking',
        choices=((True, 'Enabled'), (False, 'Disabled')),
        coerce=lambda value: str(value).lower() in ('true', '1', 'yes', 'on'),
        required=False,
        initial=True
    )
    tlpLevel = forms.ChoiceField(
        label='TLP Level',
        choices=BaseReport.tlpChoices,
        required=False,
        initial='CLEAR'
    )
    languageOverride = forms.BooleanField(
        label='Override Language from Template',
        required=False,
        initial=False
    )
    defaultLanguage = forms.ChoiceField(
        label='Report Language',
        choices=BaseReport.defaultLanguageChoices,
        required=False,
        initial='en'
    )
...
class reportForm(forms.Form):
    ...
    tlpDisplay = forms.TypedChoiceField(
        label='Show TLP on Report Pages',
        choices=((True, 'Enabled'), (False, 'Disabled')),
        coerce=lambda value: str(value).lower() in ('true', '1', 'yes', 'on'),
        initial=True,
    )
