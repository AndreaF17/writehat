...
def _parse_tlp_level(value, fallback='CLEAR'):

    return BaseReport.normalizeTlpLevel(value, fallback=fallback)


def _parse_default_language(value, fallback='en'):

    return BaseReport.normalizeDefaultLanguage(value, fallback=fallback)
...
def reportCreate(request, uuid=None, fromTemplate=False):
    ...
    try:
        ...
        tlpDisplay = _parse_bool(decodedJson.get('tlpDisplay', None), default=True)
        tlpLevel = _parse_tlp_level(decodedJson.get('tlpLevel', None), fallback='CLEAR')
        defaultLanguage = _parse_default_language(decodedJson.get('defaultLanguage', None), fallback='en')
        ...
        if uuid:
            ...
            report = Report.new(
                ...
                engagementParent=uuid,
                status=status,
                tlpDisplay=tlpDisplay,
                tlpLevel=tlpLevel,
...
def reportUpdate(request,uuid,fromTemplate=False):
    ...
    try:
        ...
        reportTlpDisplay = _parse_bool(reportJSON.get('tlpDisplay', None), default=None)
        reportTlpLevel = reportJSON.get('tlpLevel', None)
        reportDefaultLanguageRaw = reportJSON.get('defaultLanguage', None)
        reportDefaultLanguage = None
        if reportDefaultLanguageRaw is not None:
            reportDefaultLanguage = _parse_default_language(reportDefaultLanguageRaw, fallback='en')
        ...
        if fromTemplate:
            ...
            report.update(
                ...
                reportPageTemplate,
                status=reportStatus,
                tlpDisplay=reportTlpDisplay,
                tlpLevel=reportTlpLevel,
                defaultLanguage=reportDefaultLanguage,
            )
        else:
...
def engagementFgroupEdit(request,uuid):
    ...
    requested_type = request.POST.get('scoringType', fgroup.scoringType)
    requested_type = str(requested_type).strip().upper()

    if requested_type and requested_type != fgroup.scoringType:
        # Finding models differ between group types; only allow conversion when empty.
        if list(fgroup.findings):
            response = HttpResponse('Cannot change finding group type while findings exist in the group.')
            response.status_code = 400
            return response

        class_map = {
            'CVSS': CVSSFindingGroup,
            'CVSS4': CVSS4FindingGroup,
            'DREAD': DREADFindingGroup,
            'PROACTIVE': ProactiveFindingGroup,
        }
        destination_class = class_map.get(requested_type)
        if destination_class is None:
            response = HttpResponse('Invalid findings group type selected.')
            response.status_code = 400
            return response

        new_group = destination_class.new(uuid=fgroup.engagementParent, postData=request.POST)
        new_group.save()
        fgroup.delete()
        response = HttpResponse(f"Successfully changed findingsGroup type for {escape(str(uuid))}")
        response.status = 200
        return response
...
def reportCreateFromTemplate(request,uuid):
    ...
    tlpOverride = _parse_bool(request.POST.get('tlpOverride', None), default=False)
    if tlpOverride:
        report.tlpDisplay = _parse_bool(request.POST.get('tlpDisplay', None), default=report.tlpDisplay)
        report.tlpLevel = _parse_tlp_level(request.POST.get('tlpLevel', None), fallback=report.tlpLevel)

    languageOverride = _parse_bool(request.POST.get('languageOverride', None), default=False)
    if languageOverride:
        report.defaultLanguage = _parse_default_language(
            request.POST.get('defaultLanguage', None),
            fallback=report.defaultLanguage,
        )
...
def templateExport(request, uuid):
    ...
    export_data = {
        ...
        'tlpDisplay': report.tlpDisplay,
        'tlpLevel': report.tlpLevel,
        'defaultLanguage': report.defaultLanguage,
...
def templateImport(request):
    ...
    new_report.tlpDisplay = _parse_bool(data.get('tlpDisplay', None), default=True)
    new_report.tlpLevel = _parse_tlp_level(data.get('tlpLevel', None), fallback='CLEAR')
    new_report.defaultLanguage = _parse_default_language(data.get('defaultLanguage', None), fallback='en')
    new_report.save()
