function updateFindingPrefix() {
  // New finding groups should start with no prefix; users can set one manually.
  $('.modal').find('input[name="prefix"]').val('');
}
...
$(document).ready(function() {
      ...
      (confirm_callback = function () {
        $.post({
          ...
          success: function (result) {
            successRedirect(
              "/engagements",
              `Successfully deleted engagement: "${engagementName}" and all child entities`,
            );
          },
        });
      }),
  ...
  $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
    ...
    selected_status =
      $(`input[name='status'][value='${itemStatus}']:checked`).length === 1;
    ...
    loadModal("engagementEdit", function (engagementEditModal) {
      ...
      $("#engagementEditSave").click(function () {
        ...
        $.ajax({
          ...
          success: function () {
            successRedirect(
              "/engagements/edit/" + engagementID,
              "Successfully updated engagement",
            );
          },
          error: function () {
      ...
      (confirm_callback = function () {
        $.post({
          ...
          success: function (reportID) {
            successRedirect(
              "/engagements/edit/" + engagementID,
              "Successfully removed findingGroup and child findings",
            );
  ...
  $(".fgroupedit").click(function (e) {
    ...
    loadModal(
      "editFindingGroup",
      function (editFindingGroupModal) {
        ...
        $("#saveEditFgroup").click(function (e) {
          ...
          $.post({
            ...
            success: function (reportID) {
              ...
              successRedirect(
                `/engagements/edit/${engagementID}`,
                "Successfully updated finding group",
              );
            },
            error: function (result) {
              var detail = result && result.responseText ? result.responseText : "Failed to update finding group";
              error(detail);
            },
          });
        });
      ...
      $("#saveNewFgroup").click(function () {
        ...
        $.ajax({
          ...
          success: function (reportID) {
            ...
            successRedirect(
              "/engagements/edit/" + engagementID,
              "Successfully added findingGroup",
            );
            //TO DO: get load pane to work instead of a redirect
            //loadPane('findingGroupList','groupListPane');
          },
  ...
  $("#reportCreate").click(function () {
    loadModal("reportTemplateSelect", function (reportTemplateSelectModal) {
      ...
      function refreshTemplateImportTlpOverrides() {
        var overrideEnabled = $("#id_tlpOverride").is(":checked");
        $("#id_tlpDisplay").prop("disabled", !overrideEnabled);
        $("#id_tlpLevel").prop("disabled", !overrideEnabled);

        var languageOverrideEnabled = $("#id_languageOverride").is(":checked");
        $("#id_defaultLanguage").prop("disabled", !languageOverrideEnabled);
      }
      ...
      $("#id_tlpOverride").off().change(refreshTemplateImportTlpOverrides);
      $("#id_languageOverride")
        .off()
        .change(refreshTemplateImportTlpOverrides);
      refreshTemplateImportTlpOverrides();
    });
  });
  ...
  function loadReportTemplate(reportTemplateID) {
    $.post({
      ...
      data: {
        ...
        tlpOverride: $("#id_tlpOverride").is(":checked"),
        tlpDisplay: $("#id_tlpDisplay").val(),
        tlpLevel: $("#id_tlpLevel").val(),
        languageOverride: $("#id_languageOverride").is(":checked"),
        defaultLanguage: $("#id_defaultLanguage").val(),
      },
      success: function (reportID) {
